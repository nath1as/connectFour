...
class Board
  ...
  def move(num, new_state)
    state[num] << new_state
    self.add_coin(num, new_state)
    self.connected_four(num)
    @last_color = new_state
  end

  def add_coin(column, new_state)
    row = (state[column].size) -1
      @@matrix[column][row][2] = " ▅▇█▇▅ ".colorize(new_state.to_sym)
      @@matrix[column][row][1] = "▐█████▍".colorize(new_state.to_sym)
      @@matrix[column][row][0] = " ▀███▀ ".colorize(new_state.to_sym)
  end
 ...
 #negative ratio diagonal
  ...
  def over_reason
    case
    when self.won? then Gfx.prompt("#{winner.colorize(self.last_color.to_sym)} won!")
